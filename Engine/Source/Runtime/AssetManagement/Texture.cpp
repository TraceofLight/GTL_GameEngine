...
void UTexture::Load(const FString& InFilePath, ID3D11Device* InDevice, bool bSRGB)
{
...
#ifdef USE_DDS_CACHE
		// DDS 캐시 로딩 실패 시 원본 파일에서 재생성 시도
		if (FAILED(hr) && ActualLoadPath != InFilePath)
		{
			if (std::filesystem::exists(InFilePath))
			{
				...
				{
					// 재생성 성공 - 다시 로드 시도
					std::wstring WCachePath = UTF8ToWide(DDSCachePath);
					hr = DirectX::CreateDDSTextureFromFileEx(
						InDevice,
						WCachePath.c_str(),
						0,
				...
				// 재생성도 실패하면 원본 파일 직접 로드 시도 (WIC)
				if (FAILED(hr))
				{
					std::wstring WOriginalPath = UTF8ToWide(InFilePath);
	...
	{
		UE_LOG("Texture: Load: Failed %s (HRESULT: 0x%08X)", ActualLoadPath.c_str(), hr);
	}
}
