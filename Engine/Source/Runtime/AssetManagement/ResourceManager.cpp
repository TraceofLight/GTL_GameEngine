...
void UResourceManager::InitShaderILMap()
{
    ...
    ShaderToInputLayoutMap["Shaders/Sky/Sky.hlsl"] = layout;
    layout.clear();
...
void UResourceManager::CreateTextBillboardTexture()
{
    ...
    TextBillboardTexture->Load("Data/Default/Texture/TextBillboard.dds",Device);
...
void UResourceManager::ProcessLoadQueue(float MaxTimeMs)
{
	...
	if (!Loader.IsLoading() && !OnAllLoadsCompleteCallbacks.IsEmpty())
	{
		...
		OnAllLoadsCompleteCallbacks.Empty();
	}
}

void UResourceManager::RegisterOnAllLoadsComplete(std::function<void()> Callback)
{
	if (Callback)
	{
		OnAllLoadsCompleteCallbacks.Add(Callback);
	}
}
