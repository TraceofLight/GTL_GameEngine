...
IMPLEMENT_CLASS(UPrimitiveDynamicMesh)

UPrimitiveDynamicMesh::~UPrimitiveDynamicMesh()
{
    ReleaseResources();
}

bool UPrimitiveDynamicMesh::Initialize(uint32 InMaxVertices, uint32 InMaxIndices, ID3D11Device* InDevice)
{
    if (!InDevice)
        return false;
...
bool UPrimitiveDynamicMesh::UpdateData(FMeshData* InData, ID3D11DeviceContext* InContext)
{
    ...
    for (uint32 i = 0; i < vertexCount; ++i)
    {
        ...
        dstVertices[i].Color = (i < InData->Color.size()) ? InData->Color[i] : FVector4(1, 1, 1, 0.4f);
    }
...
void UPrimitiveDynamicMesh::ReleaseResources()
{
    if (VertexBuffer)
    {
