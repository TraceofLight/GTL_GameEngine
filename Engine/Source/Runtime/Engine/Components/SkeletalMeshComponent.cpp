...
void USkeletalMeshComponent::EnsurePhysicsAssetBuilt()
{
    ...
    // Create physics asset and build from skeleton
    PhysicsAsset = NewObject<UPhysicsAsset>();
    if (PhysicsAsset)
    {
        PhysicsAsset->BuildFromSkeletalMesh(SkeletalMesh);
...
void USkeletalMeshComponent::OnCreatePhysicsState()
{
    ...
    // Create a combined BodySetup with all capsules in component space
    static UBodySetup CombinedSetup;
    CombinedSetup.AggGeom.SphylElems.Empty();
    CombinedSetup.AggGeom.BoxElems.Empty();
    CombinedSetup.AggGeom.SphereElems.Empty();
